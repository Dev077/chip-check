...
Pipeline (target ~800s/benchmark):
  α₁  Focused-Poisson electrostatic GP                (mise_en_place/gp.py)        60s
  α₂  koral soft-only TILOS-faithful Adam (K=24)      (inline below)          220s
  α₃  Top-K oracle ranking + anchor safety net        (inline below)           15s
  1   FastEvaluator build (bit-exact incremental)     (mise_en_place)    10s
  2   True-cost numerical subgradient                 (mise_en_place)    60s
  3   Lin-Kernighan k-opt swap passes                 (mise_en_place)   150s
  4   Hierarchical regional LAHC (3×3 → 5×5)          (mise_en_place)   130s
  5   Global LAHC polish                              (mise_en_place)   130s
...
class GraphGradPlacer:
    ...
    def place(self, benchmark: Benchmark) -> torch.Tensor:
        ...
        if self.run_subgrad and time.time() - t0 < self.time_budget_s - self.lahc_min_budget_s:
            ...
            try:
                true_cost_subgradient(ev, time_budget_s=budget, seed=self.seed, verbose=self.verbose)
